...
(* use only representable addresses *)
Definition address : Type := block * ptrofs.
...
Lemma mapsto_valid_pointer0 : forall {CS : compspecs} b o sh t m,
  sh <> Share.bot ->
  mem_auth m ∗ mapsto_memory_block.mapsto_ sh t (Vptr b o) ⊢
  ⌜Mem.valid_pointer m b (Ptrofs.unsigned o) = true⌝.
Proof.
intros; iIntros "[Hm H]".
iAssert ⌜exists ch, access_mode t = By_value ch⌝ with "[H]" as %(ch & Hch).
{ rewrite /mapsto_memory_block.mapsto_ /mapsto_memory_block.mapsto.
  destruct (access_mode t) eqn: ?; try done.
  destruct (type_is_volatile t) eqn: ?; try done.
  eauto. }
iMod (valid_pointer.mapsto_valid_pointer1 with "H") as "H"; simpl; try done.
...
Lemma wp_pointer_cmp0: forall {CS: compspecs} E (cmp : Cop.binary_operation) ty1 ty2 v1 v2 sh1 sh2 P,
  ...
  ▷ ⎡<absorb> mapsto_memory_block.mapsto_ sh1 ty1 v1 ∧ <absorb> mapsto_memory_block.mapsto_ sh2 ty2 v2⎤ ∧
...
intros; rewrite /wp_binop.
  ...
  iDestruct (add_and _ (▷ ⌜∃ ch b o, access_mode ty1 = By_value ch ∧ v1 = Vptr b o ∧ Mem.valid_pointer m b (Ptrofs.unsigned o) = true⌝)
    with "H") as "(H & >%Hv1)".
  { iIntros "(((>H & _) & _) & Hm) !>".
    iDestruct (mapsto_memory_block.mapsto_pure_facts with "H") as %((? & ?) & ?).
    destruct v1; try contradiction.
    iDestruct (mapsto_valid_pointer0 with "[$]") as %?; eauto 7. }
...
Proof.
  intros.
  iIntros "H"; iApply (wp_pointer_cmp0 _ _ _ _ _ _ sh1 sh2); [done..|].
  iStopProof; do 5 f_equiv; iIntros "(% & H)"; iApply mapsto_memory_block.mapsto_mapsto_; iApply mapsto_weaken; done.
Qed.
...
Lemma wp_var_local : forall E f _x c_ty b (P:address->assert),
  lvar _x c_ty b ∗ (lvar _x c_ty b -∗ P (b, Ptrofs.zero))
...
Lemma wp_var_global : forall E f _x c_ty b (P:address->assert),
  ...
  ⎡gvar _x b⎤ ∗ (⎡gvar _x b⎤ -∗ P (b, Ptrofs.zero))
...
Lemma wp_deref : forall E f e ty P,
  wp_expr E f e (λ v, ∃ b o, <affine> ⌜v = Vptr b o⌝ ∗ P (b, o)) ⊢ wp_lvalue E f (Ederef e ty) P.
Proof.
...
Lemma wp_addrof : forall E f e t P,
  wp_lvalue E f e (λ '(b, o), P (Vptr b o)) ⊢ wp_expr E f (Eaddrof e t) P.
Proof.
  intros; rewrite /wp_lvalue /wp_expr.
  do 8 f_equiv.
  iIntros "(% & % & ? & $)".
...
Lemma wp_expr_ptr : forall E f e P, match access_mode (typeof e) with By_reference | By_copy => True | _ => False end →
  wp_lvalue E f e (λ '(b, o), P (Vptr b o)) ⊢ wp_expr E f e P.
Proof.
  intros; rewrite /wp_lvalue /wp_expr.
  do 8 f_equiv.
  iIntros "(% & % & ? & $)".
  iStopProof; do 7 f_equiv.
  intros ??; econstructor; eauto.
  destruct (access_mode (typeof e)) eqn: Hmode; try done.
...
Lemma wp_lvalue_field : forall E f e i t P,
  ...
  | _ => False end⌝ ∗ P (b, Ptrofs.add o (Ptrofs.repr delta))) ⊢ wp_lvalue E f (Efield e i t) P.
Proof.
  intros; rewrite /wp_lvalue /wp_expr.
  do 8 f_equiv.
  iIntros "(% & ? & $ & $ & % & % & -> & % & % & % & $)".
