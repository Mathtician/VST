...
Proof.
  split.
  - destruct x, y; intuition; try split; try pose proof (H sidx_zero) as H'; try destruct H; try destruct H'; try done.
...
Proof.
  destruct x; simpl; rewrite ucmra_unit_left_id //.
Qed.
...
Proof.
  ...
  - hnf; rewrite ucmra_unit_left_id //.
Qed.
...
Proof.
  ...
  - intros ??.
    ...
    + destruct (dfrac_error _) eqn: Herr; first by rewrite Hop1 Hop2.
      ...
      hnf; by rewrite (cmra_comm(A := shareR)).
  - intros [|].
    + rewrite /op /shared_op_instance /core /pcore /shared_pcore_instance /=.
      destruct dq.
      * rewrite /ε /shared_unit_instance ucmra_unit_right_id.
        destruct (readable_dfrac_dec _); done.
  ...
  - intros ?? (z & H).
    ...
    + destruct dq; first by eexists; rewrite left_id.
      ...
      * destruct Hop as (? & Hval & Hop).
        ...
        rewrite -agree_included; eapply (cmra_included_proper(A := agreeR V)); try done.
         rewrite -Some_included_total -Hval; eexists; done.
...
Proof.
  intros ?? [-> | <-]; try done.
  destruct dq as [[|]|[|]]; try done; simpl; rewrite ucmra_unit_right_id //.
Qed.
...
Proof.
  ...
  - intros ??? [H | [Hd Hv]].
    ...
    + right; destruct Hd as [<- | <-], dq; rewrite ?dfrac_op_own_discarded ?dfrac_op_both_discarded // /=.
      split.
      * right; rewrite ucmra_unit_left_id //.
  ...
  - split.
    ...
    + intros H; pose proof (H sidx_zero) as H0; destruct H0 as [? | [??]]; first by left.
      right; split; try done.
    ...
    + destruct Hop as (? & Hv & ->).
      ...
      * right; destruct dq, cx; inversion Heq; subst; simpl.
        -- destruct (_ ⋅ _); try done.
           split; first by right; rewrite ucmra_unit_left_id.
           apply agree_increasing.
